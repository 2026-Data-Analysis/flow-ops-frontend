...
export function RegistrationPage() {
  ...
  const [availableBranches, setAvailableBranches] = useState<string[]>(commonBranches);
  ...
  const handleRepoSelect = async (repo: Repository) => {
    ...
    try {
      ...
      const initiallySelected = branches
        .filter((branch) => branch.selected || branch.isDefault)
      ...
      setSelectedBranches((prev) => (prev.length > 0 ? prev : ['main']));
    } finally {
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        >
          ...
          {selectedRepo && !scanComplete && (
            <button
              onClick={handleScan}
              disabled={isScanning || isLoadingBranches || selectedBranches.length === 0}
